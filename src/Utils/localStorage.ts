...
export type storeMultipleLocalDataType = [LocalStorageKey,object];
export const storeMultipleLocalData = async(storageDataArray:storeMultipleLocalDataType[])=>{
    try {
        let dataReadyTostore = storageDataArray.map((item)=>(
            [item[0],JSON.stringify(item[1])]
        ))as readonly (readonly [string, string])[];
        
        await AsyncStorage.multiSet(dataReadyTostore);
    } catch (error) {
        console.log('error from storeDataToLocal',error);
    }
}
